...
def find_methods_in_package(package, test, sortkey=None):
    ...
    if sortkey == None:
        def sortkey(x): return x.__name__
    ...
    def obtain_graph(args, suffix=""):
        ...
        if getattr(args, 'layer' + suffix) is not None \
            ...
            raise ArgumentParsingException(
                "argument --layer can only be used when argument --permutation is set as well.")
        ...
        if getattr(args, 'tree'+suffix) is not None:
            ...
            raise RuntimeError(
                "Command line does not specify a directed acyclic graph")
