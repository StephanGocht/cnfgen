...
def _shuffle_literals(constraint, substitution):
    ...
    if type(constraint) == disj:
        ...
        terms = ((w*substitution[v]//abs(substitution[v]),
                  abs(substitution[v])) for (w, v) in constraint)
        ...
        if type(constraint) == weighted_eq:
            ...
            ValueError("The constraint type is unknown: {}".format(
                type(constraint)))
    else:
        raise ValueError(
            "The constraint type is unknown: {}".format(type(constraint)))
...
def Shuffle(F, **kwargs):
    ...
    for (old, cnst) in enumerate(F._constraints):
        out._constraints[constraints_permutation[old]
                         ] = _shuffle_literals(cnst, substitution)
