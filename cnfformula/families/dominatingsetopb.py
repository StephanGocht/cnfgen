...
class DominatingSetCmdHelper(object):
    ...
    def setup_command_line(parser):
        ...
        group.add_argument('--regular', action='store_true',
                           help="Set size to V/(deg+1)")
        group.add_argument('--tiling', action='store_true',
                           help="Add tiling constraints")
        parser.add_argument('--seed', action='store_true',
                            help="Set some vertex to true")
    ...
    def build_cnf(args):
        ...
        D = args.d
        if args.regular:
            D = G.order()/(2*G.number_of_edges()/G.order()+1)
