...
class TestGraphIO(unittest.TestCase):
    ...
    def test_readGraph_dimacs_path2(self):
        ...
        self.assertRaises(ValueError, readGraph, sio(
            dimacs_path2), graph_type='simple')
        G = readGraph(sio(dimacs_path2), graph_type='simple',
                      file_format='dimacs')
        self.assertEqual(G.order(), 3)
        self.assertEqual(len(G.edges()), 2)
    ...
    def test_undoable_io(self):
        ...
        self.assertRaises(IOError, readGraph,
                          "does_not_exist.gml", graph_type='simple')

        # assumes that '/does_not_exist.gml' is not writable
        self.assertRaises(IOError, writeGraph, nx.Graph(),
                          "/does_not_exist.gml", graph_type='simple')
